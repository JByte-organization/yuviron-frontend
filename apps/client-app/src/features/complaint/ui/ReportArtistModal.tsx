...
import { usePostApiComplaints } from '@repo/api/client.ts'; // Наша общая Orval-мутация
...
const REASON_LABELS: Record<keyof typeof ComplaintReasonCode, string> = {
    Spam: "Спам / Фейковий акаунт артиста",
    CopyrightViolation: "Порушення інтелектуальної власності / Плагіат",
    Pornography: "Невідповідний вміст в обкладинках або біографії",
    HateSpeech: "Пропаганда ворожнечі або насильства у релізах",
    Abuse: "Порушення правил платформи",
    Scam: "Шахрайство / Видавання себе за іншого виконавця",
    Other: "Інша причина"
};
...
export const ReportArtistModal = ({ isOpen, onClose, artistId, artistName, onSuccess }: ReportArtistModalProps) => {
    ...
    const onSubmit = async (values: ReportFormValues) => {
        try {
            // Формируем CreateComplaintCommand со схемой TargetType = Artist
            await sendComplaint({
                data: {
                    targetType: 'Artist',
                    targetId: artistId,
            ...
            console.error('[Artist Complaint Error] Не вдалося надіслати скаргу на артиста:', error);
        }
    };

    return (
        <Modal isOpen={isOpen} onClose={handleClose} title={`Поскаржитися на виконавця ${artistName}`}>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="mb-3">
                    <label className="client-modal__field-label mb-1">Пояснення скарги (необов'язково)</label>
                    <textarea
                        ...
                        placeholder="Опишіть, що саме порушує цей виконавець..."
