...
import { usePostApiComplaints } from '@repo/api/client.ts'; // Загальний Orval-мутатор скарг
...
const REASON_LABELS: Record<keyof typeof ComplaintReasonCode, string> = {
    Spam: "Спам / Фейковий реліз",
    CopyrightViolation: "Порушення моїх авторських прав (плагіат)",
    Pornography: "Невідповідний вміст обкладинки чи назв",
    HateSpeech: "Мова ворожнечі / Заклики до насильства",
    Abuse: "Шкідливий вміст / Образи",
    Scam: "Шахрайство або викрадення чужого релізу",
    Other: "Інша причина"
};
...
export const ReportAlbumModal = ({ isOpen, onClose, albumId, albumTitle, onSuccess }: ReportAlbumModalProps) => {
    ...
    const onSubmit = async (values: ReportFormValues) => {
        try {
            ...
            console.error('[Album Complaint Error]', error);
        }
    };

    return (
        <Modal isOpen={isOpen} onClose={handleClose} title={`Поскаржитися на альбом "${albumTitle}"`}>
