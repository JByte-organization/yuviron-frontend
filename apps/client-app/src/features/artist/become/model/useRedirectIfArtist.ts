...
// (artistId резолвиться насамперед з managedArtists у /auth/me, далі JWT-claim/
// localStorage), форми claim/create йому не потрібні — відправляємо в студію.
// Бонус: коли admin схвалює claim, артист зʼявляється в managedArtists → юзер,
// що зайшов на /become-artist, автоматично перекидається в кабінет. Закриває як
// вхід через /become-artist, так і deep-link на /become-artist/{claim,create}.
//
