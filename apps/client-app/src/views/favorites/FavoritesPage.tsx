...
import { useGetApiMeFavoritesTracks, useGetApiAuthMe, type UserFavoriteTrackDto, type TrackArtistDto } from '@repo/api/client.ts';
import { getImageUrl } from '@/shared/lib/getImageUrl';
import { PlaylistHeader } from '@/shared/ui/Playlist/PlaylistHeader/PlaylistHeader';
import { useTrackCollection } from '@/shared/lib/useTrackCollection';
import type { TrackRowData } from '@/entities/track/ui/TrackRow';
import type { UserFavoriteTrackDtoPaginatedList } from "@repo/api/generated/client/models";

export const FavoritesPage = () => {
    const { data: favoritesRaw, isLoading } = useGetApiMeFavoritesTracks({ PageSize: 100 });
    ...
    return (
        <div className="favorites-page py-4">
            <PlaylistHeader
                breadcrumb="Плейліст" title="Улюблені треки" systemName={isUserLoading ? 'Завантаження...' : userName}
                totalCount={processedTracks.length} isCollectionPlaying={isCollectionPlaying}
                isSearchExpanded={isSearchExpanded} setIsSearchExpanded={setIsSearchExpanded}
                search={search} setSearch={setSearch} sortOrder={sortOrder} sortButtonLabel={sortButtonLabel}
            ...
            <div className="favorites-page__list mt-2">
                {isLoading ? <FavoritesSkeleton /> : processedTracks.length === 0 ? (
                    <div className="favorites-page__empty text-center py-5"><p className="text-secondary mt-2">Нічого не знайдено</p></div>
                ) : (
                    processedTracks.map(track => (
                        <TrackRow key={track.id} track={track} allTracks={processedTracks} sourceType="Playlist" sourceId="favorites" />
                    ))
                )}
            </div>
        </div>
