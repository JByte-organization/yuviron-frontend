'use client';

import React, { useEffect } from 'react';
import { useForm } from 'react-hook-form';
import { useQueryClient } from '@tanstack/react-query';
import {
    useGetApiAdminPlansId,
    usePutApiAdminPlansId,
    PlanPeriod,
    getGetApiAdminPlansIdQueryKey,
    type PlanListItemDto,
    type PlanDetailsDto
} from '@repo/api/admin.ts';

interface EditPlanModalProps {
    plan: PlanListItemDto;
    isOpen: boolean;
    onClose: () => void;
    onSuccess: () => void;
}

type FormValues = {
    name: string;
    price: number;
    currency: string;
    period: typeof PlanPeriod[keyof typeof PlanPeriod];
};

export const EditPlanModal = ({ plan, isOpen, onClose, onSuccess }: EditPlanModalProps) => {
    const planId = plan.id ?? '';
    const queryClient = useQueryClient();

    const { register, handleSubmit, reset, setError, formState: { errors, isSubmitting } } = useForm<FormValues>({
        mode: 'onChange'
    });

    const { data: detailsRaw, isLoading } = useGetApiAdminPlansId(planId, {
        query: {
            queryKey: getGetApiAdminPlansIdQueryKey(planId),
            enabled: isOpen && !!planId
        }
    });

    const { mutateAsync: updatePlan, isPending } = usePutApiAdminPlansId();
    const details = (detailsRaw as { data?: PlanDetailsDto } | undefined)?.data ?? (detailsRaw as PlanDetailsDto | undefined);

    useEffect(() => {
        if (!details) return;
        reset({
            name: details.name ?? '',
            price: details.price ?? 0,
            currency: details.currency ?? 'USD',
            period: details.period ?? PlanPeriod.Month
        });
    }, [details, reset]);

    const onSubmit = async (values: FormValues) => {
        try {
            await updatePlan({
                id: planId,
                data: {
                    name: values.name.trim(),
                    price: Number(values.price),
                    currency: values.currency,
                    period: values.period,
                }
            });

            await queryClient.invalidateQueries({
                queryKey: getGetApiAdminPlansIdQueryKey(planId)
            });

            onSuccess();
            onClose();
        } catch {
            setError('root', { message: 'Failed to rewrite premium cluster grid context.' });
        }
    };

    if (!isOpen) return null;

    return (
        <div className="modal d-block" style={{ backgroundColor: 'rgba(0,0,0,0.85)', zIndex: 1050 }}>
            <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content bg-admin-primary border border-secondary shadow-lg text-white">
                    <div className="modal-header border-secondary p-4">
                        <div>
                            <h5 className="modal-title fw-bold text-cyan mb-0">Modify Tariff Tier</h5>
                            <small className="text-secondary">Plan ID: {planId}</small>
                        </div>
                        <button type="button" className="btn-close btn-close-white" onClick={onClose} />
                    </div>

                    {isLoading ? (
                        <div className="modal-body p-5 text-center">
                            <div className="spinner-border text-info" />
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="modal-body p-4 d-flex flex-column gap-3" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                                {errors.root && <div className="alert alert-danger py-2 small">{errors.root.message}</div>}

                                <div>
                                    <label className="form-label text-secondary small fw-bold">PLAN TITLE *</label>
                                    <input type="text" className="form-control admin-login__input" {...register('name', { required: true })} />
                                </div>

                                <div className="row">
                                    <div className="col-md-6">
                                        <label className="form-label text-secondary small fw-bold">PRICE AMOUNT *</label>
                                        <input type="number" step="0.01" className="form-control admin-login__input font-monospace" {...register('price', { required: true })} />
                                    </div>
                                    <div className="col-md-6">
                                        <label className="form-label text-secondary small fw-bold">CURRENCY</label>
                                        <select className="form-select admin-login__input text-white" {...register('currency')}>
                                            <option value="USD">USD ($)</option>
                                            <option value="EUR">EUR (€)</option>
                                            <option value="UAH">UAH (₴)</option>
                                        </select>
                                    </div>
                                </div>

                                <div>
                                    <label className="form-label text-secondary small fw-bold">BILLING CYCLE</label>
                                    <select className="form-select admin-login__input text-white" {...register('period')}>
                                        <option value={PlanPeriod.Month}>Monthly</option>
                                        <option value={PlanPeriod.Year}>Yearly</option>
                                    </select>
                                </div>
                            </div>

                            <div className="modal-footer border-0 p-4">
                                <button type="button" className="btn btn-admin-dark px-4" onClick={onClose}>Cancel</button>
                                <button type="submit" className="btn btn-primary px-5 fw-bold" disabled={isPending || isSubmitting}>
                                    {isPending ? 'Saving...' : 'Save Changes'}
                                </button>
                            </div>
                        </form>
                    )}
                </div>
            </div>
        </div>
    );
};