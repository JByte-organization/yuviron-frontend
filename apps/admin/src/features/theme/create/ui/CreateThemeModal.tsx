...
import { usePostApiAdminThemes, type CreateThemeCommand } from '@repo/api/admin.ts';

interface Props { isOpen: boolean; onClose: () => void; onSuccess: () => void; }

export const CreateThemeModal = ({ isOpen, onClose, onSuccess }: Props) => {
    const { register, handleSubmit, reset, setError, watch, formState: { errors } } = useForm<CreateThemeCommand>({
        mode: 'onChange',
        defaultValues: { name: '', primaryColor: '#7AE0FF', secondaryColor: '#1D4ED8', backgroundColor: '#0B0C12', isPremiumOnly: true }
    });

    const { mutateAsync: createTheme, isPending } = usePostApiAdminThemes();

    // Живий мапінг віпорту прев'ю
    ...
    const onSubmit = async (values: CreateThemeCommand) => {
        try {
            // 🚨 ВІДПРАВЛЯЄМО ДАНІ СУВОРO ЗА КOНТРАКТOМ CreateThemeCommand
            await createTheme({
                data: {
            ...
            setError('root', { message: 'Failed to deploy design template entity.' });
        }
    };
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg">
                <div className="modal-content bg-admin-primary border border-secondary text-white shadow-lg">
                    <div className="modal-header border-secondary p-4">
                        <h5 className="modal-title fw-bold text-cyan">Deploy Official Ambient Theme</h5>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)}>
                        <div className="modal-body p-4 d-flex flex-column gap-4">
                            ...
                            <div className="row g-3">
                                <div className="col-md-8">
                                    ...
                                    <input type="text" className={`form-control admin-login__input ${errors.name ? 'is-invalid' : ''}`} placeholder="e.g. Sunset Glow" {...register('name', { required: 'Theme title required' })} disabled={isPending} />
                                </div>
                            ...
                            <div className="row align-items-center g-4">
                                <div className="col-md-6">
                                    <label className="form-label text-secondary small fw-bold">VOLUMETRIC PREVIEW CHROMATIC MAP</label>
                                    <div
                                    ...
                                    >
                                        <div>
                                            <span className="d-block fw-bold text-white small">Active Client Canvas Layer</span>
                                            <span className="text-white-50 font-monospace" style={{ fontSize: '10px' }}>Realtime CSS map matching layout</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="col-md-6 d-flex flex-column gap-3">
                                    <div className="d-flex align-items-center gap-3">
                                        ...
                                        <span className="small text-secondary fw-bold">Primary Accent Color</span>
                                    </div>
                                    <div className="d-flex align-items-center gap-3">
                                    ...
                                    <div className="d-flex align-items-center gap-3">
                                        ...
                                        <span className="small text-secondary fw-bold">Deep Ground Base Canvas</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="modal-footer border-secondary border-opacity-20 p-4">
                            ...
                            <button type="submit" className="btn btn-primary px-5 fw-bold" disabled={isPending}>Deploy Nodes</button>
                        </div>
                    </form>
                </div>
            </div>
