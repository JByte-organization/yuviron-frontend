...
export const DeleteMoodModal = ({ mood, isOpen, onClose, onSuccess }: Props) => {
    const { mutateAsync: deleteMood, isPending } = useDeleteApiAdminMoodsId();

    const handleDelete = async () => {
        if (!mood?.id) return;

        try {
            await deleteMood({ id: mood.id });
            onSuccess();
            onClose();
        } catch (error: any) {
            const status = error.response?.status;
            alert(`Error ${status}: Failed to delete mood.`);
        }
    };
            ...
            <div className="modal-dialog modal-dialog-centered modal-sm">
                <div className="modal-content bg-admin-primary border border-danger shadow-lg text-white">
                    ...
                    <div className="modal-header border-secondary p-4">
                        <h5 className="modal-title fw-bold text-danger">Delete Mood</h5>
                    ...
                    <div className="modal-body p-4 text-center">
                        ...
                        <p className="fw-bold text-danger fs-5 mb-1">"{mood.name}"</p>
